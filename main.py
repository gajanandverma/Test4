...
with st.sidebar:
    ...
    <div style="color: black;">
        <ol>
            <li>Enter your <a href="https://platform.openai.com/account/api-keys">OpenAI API key</a> and chosen model below 🔑</li>
            <li>Provide details of the application that you would like to threat model 📝</li>
            <li>Generate a threat list, attack tree and/or mitigating controls for your application 🚀</li>
        </ol>
    </div>
    """,
...
with st.sidebar:
    ...
    No, the threat models are not 100% accurate. TARA uses GPT Large Language Models (LLMs) to generate its output. The GPT models are powerful, but they sometimes makes mistakes and are prone to 'hallucinations' (generating irrelevant or inaccurate content). Please use the output only as a starting point for identifying and addressing potential security risks in your applications.
    """
    )
    st.markdown(
        """
